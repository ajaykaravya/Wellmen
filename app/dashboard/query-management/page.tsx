...
function QueryListContent() {
  ...
  const loadQueries = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/query-management?${params.toString()}`);
      if (!res.ok) {
  ...
  }, [debouncedQuery, pageIndex, pageSize, statusFilter]);
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/query-management/${confirmTarget.id}`, {
        method: "DELETE",
      });
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="rbac-inline-actions flex gap-4">
            <Link href={`/dashboard/query-management/${row.original.id}`}>
                  ...
                  <div key={item.id} className="rbac-card p-4">
                    <div className="mb-2 flex items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        <Link href={`/dashboard/query-management/${item.id}`}>
                          <button className="rbac-link" type="button">
                            <FaEdit />
                          </button>
                        </Link>
